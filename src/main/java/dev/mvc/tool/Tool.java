...
public class Tool {
   ...
   */
  public static synchronized String preview(String upDir, String _src, int width, //언더바(_)붙이는 이유는 같은 소스이지만 목적이 다를때.
      int height) {
    int RATIO = 0;
    int SAME = -1;
  ...
  public static synchronized String checkNull(String str) {
    if (str == null) {
      str = ""; //nullpointexception을 막기위해 사용
    }
    ...
    try {
      if (folder != null && fileName != null) { // 값이 있는지 확인
        ...
        if (file.exists() && file.isFile()) { // 존재하는 파일인지 검사
          System.out.println("삭제 할 파일 존재 함");
  ...
  public static String getRandomDate(){
    ...
    date = date + "_" + (int)(Math.random()*1000); //random은 실수이기 때문에, 1000을 곱함
