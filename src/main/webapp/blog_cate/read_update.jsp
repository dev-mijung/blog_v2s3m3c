...
<jsp:include page="/menu/top.jsp" />
  ...
  <div id='panel_create' style='padding: 10px 0px 10px 0px; background-color: #F9F9F9; width: 100%; text-align: center;'>
    <form name='frm_create' id='frm_create' method='POST' action='./read_update.do'>
      ...
      <label>그룹 번호</label>
      <input type='number' name='blog_categrpno' value='${blog_Cate_VO.blog_categrpno }' required='required'
                min='1' max='99999' step='1' style='width: 5%;'>
                
      <label>카테고리</label>
      <input type='text' name='cate_name' value='${blog_Cate_VO.cate_name }' required='required'
                style='width: 25%;'>
      ...
      <select name='cate_visible'>
        <option value='Y' ${blog_Cate_VO.cate_visible == 'Y' ? "selected='selected'" : "" }>Y</option>
        <option value='N' ${blog_Cate_VO.cate_visible == 'N' ? "selected='selected'" : "" }>N</option>
      </select>
      
     <%--  <label>등록 글 수</label>
      <input type='number' name='cate_cnt' value='${blog_Cate_VO.cate_cnt }' required='required'
